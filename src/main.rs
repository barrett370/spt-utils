    ...
    match get_token(&mut oauth).await {
        Some(token_info) => {
            ...
            if let Some(_) = matches.subcommand_matches("get-currently-playing") {
                println!("{:?}",get_currently_playing(spotify).await);
...
async fn get_currently_playing(spotify:Spotify) -> String { 

            let playing = spotify.current_user_playing_track().await;
            match playing {
                Err(e) => format!("no result, {:?}", e),
                Ok(p) => {
                    match p {
                        None => return String::new(),
                        Some(p) => {
                            let track = p.item.unwrap();
                            return format!("{} - {}", track.name, track.artists.get(0).unwrap().name);
                        }
                    }
                }
            }
